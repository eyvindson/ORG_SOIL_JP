...
# Here are some QA/QC functions


# This function checks that a full time series of values is found from 1990 to [current inventory year]

# FUNC_QA_checkmaxyear <- function(data) {
#   # Only applies to dataframes with the column "year" present
#   if("year" %in% colnames(data)) {
#     maxyear = max(data$year)
#     if(maxyear != CONST_inventory_year) {
#       stop(paste("*** WARNING *** time series final year mismatch! Final year should be:",
#                   CONST_inventory_year, 
#                   "Final year of current dataset:", maxyear))
# 
#     }
#   }
# }
# 
# # This function checks for NA 
# 
# FUNC_QA_nacheck <- function(data) {
#  
#   if(anyNA(data)) {
#     print("*** WARNING *** NAs found in the data")
#   }
# }
# 
# 
...
# 
# 
# testframe <- data.frame(year = rep(yearz, 3), 
#                         cat = rep(c("A", "B", "C"), each = length(yearz)),
#                         val = rep(rnorm(length(yearz*3), 1, 1)))
#                         
# 
# hasstur <-
#   testframe %>% 
