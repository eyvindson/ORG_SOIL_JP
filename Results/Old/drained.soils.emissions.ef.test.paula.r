##2.2.2017: Paula is testing the emission factors, how they compare to WL Supplement ones. Removing the effect of deadwood and adding aboveground litter
...
###Annual litter flux to aboveground
...
nwl.bel <- nwl.sf.nfi9013.org.bel+nat.nwl.SF.org.bel[21:dim(nat.nwl.SF.org.bel)[1], ]+t(matrix(und.org.bel))
fwl.bel <- fwl.sf.nfi9013.org.bel+nat.fwl.SF.org.bel[21:dim(nat.fwl.SF.org.bel)[1], ]
...
litter.abv.sf <- rowSums(nwl.abv+fwl.abv)
...
Rhtkg.emis.sf <- (litter.bel.sf+litter.abv.sf-Rhtkg)
Mtkg.emis.sf <- (litter.bel.sf+litter.abv.sf-Mtkg)
Ptkg.emis.sf <- (litter.bel.sf+litter.abv.sf-Ptkg)
Vatkg.emis.sf <- (litter.bel.sf+litter.abv.sf-Vatkg)
Jatkg.emis.sf <- (litter.bel.sf+litter.abv.sf-Jatkg)


## Writing tkg emissions

tkg.emiss.sf <- rbind(Rhtkg.emis.sf,Mtkg.emis.sf,Ptkg.emis.sf,Vatkg.emis.sf,Jatkg.emis.sf)
#write.csv(tkg.emiss.sf, (paste(resultspath,"tkg.emiss.sf.csv", sep="")))
# NIR appendix
...
litter.abv.nf <- rowSums(nwl.abv+fwl.abv)
...
Rhtkg.emis.nf <- (litter.bel.nf+litter.abv.nf-Rhtkg)
Mtkg.emis.nf <- (litter.bel.nf+litter.abv.nf-Mtkg)
Ptkg.emis.nf <- (litter.bel.nf+litter.abv.nf-Ptkg)
Vatkg.emis.nf <- (litter.bel.nf+litter.abv.nf-Vatkg)
Jatkg.emis.nf <- (litter.bel.nf+litter.abv.nf-Jatkg)
...
#write.table(LU4A21.organic.soil, paste(crf,"LU4A1_FL-FL_Organic_soil.csv", sep=""), sep=" ", row.names = TRUE, col.names = FALSE, quote=FALSE)
