...
maakunnat <- FUNC_read_file(PATH_lookup_maku) %>%  select(-maakunta_nimi)
...
basal_area_by_treetype <- FUNC_read_file(PATH_basal_areas_by_treetype)
...
living_tree_litter <- FUNC_read_file(PATH_living_tree_litter) %>% 
  group_by(maakunta, year, peat_type) %>% 
...
ground_vegetation_litter <-
  ...
  select(maakunta, peat_type, year, ground_vegetation_litter)
...
fine_root_litter <-
  ...
  left_join(maakunnat) %>% 
  left_join(CONST_fine_root_biomass_region %>% mutate(region = if_else(region == "south", 1, 2))) %>% 
  ...
  select(maakunta, peat_type, year, fine_root_biomass) %>% 
  ...
  select(maakunta, peat_type, year, fine_root_litter_production)
...
total_living_litter <-
  ...
  select(maakunta, year, peat_type, total_living_litter)
...
if(PARAM_draw_plots) {
  ...
  total_litter_plot <-
    ...
    left_join(ground_vegetation_litter) %>% 
    left_join(fine_root_litter) %>% 
    pivot_longer(names_to = "litter_compartment",
                 cols = "living_tree_litter":"fine_root_litter_production")
  
  fig <- ggplot(data=total_litter_plot, aes(x = year, y = value, col = litter_compartment)) +
    geom_area(aes(fill = litter_compartment)) + 
    ylab("tons of C / ha / y") +
    labs(title = "Litter production") + 
    ylim(0, NA) +
    facet_grid(peat_type~maakunta) +
    theme_bw()
  ggsave(fig, filename = file.path(PATH_figures, "living_litter.png"), dpi = 120)
